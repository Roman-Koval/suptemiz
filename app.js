...
// ========== ПЕРЕВОДЫ ==========
const translations = {
    ru: { cancel: "Отменить", review_placeholder: "Ваш отзыв...", send_review: "Отправить отзыв", add_photo: "Добавить фото", order_btn: "Оформить + Оплатить", save_profile: "Сохранить профиль", orders_title: "Заказы", history_title: "История" },
    en: { cancel: "Cancel", review_placeholder: "Your review...", send_review: "Submit review", add_photo: "Add photo", order_btn: "Order + Pay", save_profile: "Save profile", orders_title: "Orders", history_title: "History" },
    tr: { cancel: "İptal", review_placeholder: "Yorumunuz...", send_review: "Yorum gönder", add_photo: "Fotoğraf ekle", order_btn: "Sipariş + Öde", save_profile: "Profili kaydet", orders_title: "Siparişler", history_title: "Geçmiş" }
};
...
async function createOrder() {
    ...
    const order = {
        ...
        orderDate: selectedDate, orderTime: document.getElementById('time')?.value || '09:00 - 12:00',
...
function loadHistory() {
    ...
    let display = (currentRole === 'client') ? allOrders.filter(o => o.userPhone === userPhone) : allOrders;
    ...
    container.innerHTML = display.map(order => {
        ...
        return `
            <div class="history-item">
                ...
                ` : (canCancel ? `<div><button class="btn-small" onclick="updateOrderStatus(${order.id}, 'Отменён')">❌ ${t('cancel')}</button></div>` : '')}
                ${canReview ? `
...
function renderNavigation() {
    ...
    nav.innerHTML = '';
    if (!isStaff) {
        // Клиент: Заказ + История + Профиль
        const btns = [
            { id: 'order', label: '📋 Заказ', tab: 'order-tab' },
            { id: 'history', label: `📜 ${t('history_title')}`, tab: 'history-tab' },
            { id: 'profile', label: '👤 Профиль', tab: 'profile-tab' }
        ];
        btns.forEach(btn => {
            const button = document.createElement('button');
            button.className = 'nav-tab';
            button.dataset.tab = btn.id;
            button.textContent = btn.label;
            button.onclick = () => switchTab(btn.id, btn.tab);
            nav.appendChild(button);
        });
        // Показываем вкладку заказа
        document.getElementById('order-tab').style.display = 'block';
    } else {
        // Админ/Клинер: Заказы (все) + Профиль
        const btns = [
            { id: 'history', label: `📋 ${t('orders_title')}`, tab: 'history-tab' },
            { id: 'profile', label: '👤 Профиль', tab: 'profile-tab' }
        ];
        btns.forEach(btn => {
            const button = document.createElement('button');
        ...
        // Скрываем вкладку заказа для персонала
...
function checkRoleAndRenderNav() {
    const phone = localStorage.getItem('suptemiz_userPhone');
